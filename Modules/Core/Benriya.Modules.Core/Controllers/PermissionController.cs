...
namespace Benriya.Modules.Core.Controllers
{
    ...
    public class PermissionController : CommonController
    {
        ...
        public async Task<ActionResult> Index(int id)
        {
            ...
            var data = await _storage.GetRepository<IPermission_Repository>().GetAsync(x => x.id == id);
            if (data != null)
            {
                result.Data = data;
                return Ok(result);
        ...
        {
            var result = new ApiPagingtModel<List<Permission_Access>>();
            if (pageSize > 100)
            {
                result.BadRequest("Limited 100 rows");
                return BadRequest(result);
            }
            var data = _storage.GetRepository<IPermission_Repository>().GetList(new PagingParams() { PageNumber = page, PageSize = pageSize }, new SearchOptions());
        ...
        public async Task<ActionResult> Remove(int id)
        {
            ...
            var data = await this._storage.GetRepository<IPermission_Repository>().RemoveAsync(id);
            if (data)
            {
                result.Data = data;
                return Ok(result);
        ...
        {
            var result = new ApiResultModel<bool>();
            var data = await this._storage.GetRepository<IPermission_Repository>().DeleteAsync(id);
            if (data)
            {
                result.Data = data;
                return Ok(result);
